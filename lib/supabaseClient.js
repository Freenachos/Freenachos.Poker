  ...
  async create(article) {
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        send_newsletter: article.sendNewsletter || false
      }])
      .select()
      .single();
...
export const subscribersApi = {
  // Get all confirmed subscribers (for admin view)
  async getAll() {
    const { data, error } = await supabase
      .from('subscribers')
      .select('*')
  ...
  // Subscribe (client-side - prefer using the API route)
  async subscribe(email, source = 'popup') {
    const { data, error } = await supabase
      .from('subscribers')
